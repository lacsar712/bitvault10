...
import (
	...
	"example.com/bitvault/internal/util/filedump"
	"example.com/bitvault/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "Bitvault A/B", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "bitvault"},
	}
